...
SELECT * FROM BWOOD WHERE LAST_NAME LIKE '%AN';					# % ==> n number of characters
SELECT * FROM BWOOD WHERE LAST_NAME LIKE '%A_';					# _ ==> only 1 character
...
--MATHS FOR COLUMN
SELECT SUM(AGE) FROM BWOOD;
SELECT MAX(AGE) FROM BWOOD;
SELECT MIN(AGE) FROM BWOOD;
SELECT AVG(AGE) FROM BWOOD;
...
--- LEFT (below will give ONLY 2 LETTERS FROM LEFT SIDE)
SELECT LEFT(FIRST_NAME,2) FROM BWOOD

---RIGHT (BELOW WILL GIVE ONLY 2 LETTERS FROM RIGHT SIDE)
...
SELECT *,UPPER(FIRST_NAME) MOTU FROM BWOOD
...
--difference between Union and Union All is that Union will remove duplicates values while 
--Union All will not removes duplicate rows or records, instead, it just selects all the rows from 
--all the tables
--UNION ALL
SELECT AGE FROM BWOOD
UNION ALL
...
-- TO MODIFY EXISTING ROW
UPDATE BWOOD  SET FIRST_NAME='BHAVIN',LAST_NAME='SHAH',COUNTRY_STATE='INDIA',AGE=55,CITY='BANGALORE'
WHERE FIRST_NAME='SR' AND LAST_NAME='KHAN';
...
-- create STORED PROCEDURE (run and verify)
CREATE PROCEDURE akhil2()
BEGIN
